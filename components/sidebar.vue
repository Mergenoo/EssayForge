<template>
  <aside
    class="fixed top-16 left-0 w-64 h-[calc(100vh-4rem)] bg-[#0f0f0f] text-white border-r border-white/10 overflow-y-auto flex flex-col"
  >
    ...
    <div class="flex flex-col flex-grow">
      ...
      <!-- Folders -->
      <div
        class="mt-6 px-4 text-xs text-gray-400 uppercase flex items-center justify-between shrink-0"
      >
        <span>Folders</span>
        <div class="flex gap-2">
          <i-lucide-sort-asc class="w-4 h-4" />
          <i-lucide-search class="w-4 h-4" />
          <button class="@click"><i-lucide-plus class="w-4 h-4" /></button>
        </div>
      </div>
      <div class="text-xs text-gray-500 px-4 mt-2 shrink-0">
        You haven't created any folders yet. Add a folder above to start
        organizing your work.
      </div>

      <!-- Spacer to push the bottom block down -->
...
import type { folderts } from "~/types/users";
...
const createFoler = async () => {
  if (!user.value) return;

  const { error } = await client.from("folder").inser(folderts as any);
};
